...
export default async function EventDetailPage({
  ...
  // [공통 컴포넌트] 카드 헤더
  const CardHeader = ({ icon: Icon, title, rightElement }: { icon: any, title: string, rightElement?: React.ReactNode }) => (
    ...
    <div className="min-h-screen bg-[#F8F9FA] py-8">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mb-6 flex items-center justify-between">
          <Link href="/community/events" className="group flex items-center text-sm font-medium text-slate-500 hover:text-slate-900 transition-colors">
        ...
        <div className="flex flex-col gap-8">
          ...
          {/* [ROW 1] 
            왼쪽: 썸네일/헤더
            오른쪽: 신청 카드
            -> 이 두 카드는 높이가 달라도 상관 없습니다. Row 2 시작점은 가장 긴 카드를 기준으로 맞춰집니다.
          */}
          ...
          {/* [ROW 2] - 여기가 핵심입니다!
            Row를 새로 시작했기 때문에, 위쪽 카드들의 높이가 달라도
            여기 있는 [상세 내용]과 [호스트 소개] 카드는 무조건 같은 Y축(높이)에서 시작합니다.
          */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                {attendees && attendees.length > 0 ? (
                  <div className="flex flex-wrap gap-4">
                    {attendees.map((attendee, index) => {
                      ...
                      return (
                        <div key={index} className="flex flex-col items-center gap-2 w-16 group cursor-default">
