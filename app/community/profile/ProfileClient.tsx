...
type ProfileClientProps = {
  ...
  createdEvents: EventListItem[]
  userPosts: PostListItem[]
  registeredEvents: EventListItem[]
  visibleBadges: VisibleBadge[]
}
...
function PostListItemComponent({ post }: { post: PostListItem }) {
  return (
...
function EventListItemComponent({ event }: { event: EventListItem }) {
...
export function ProfileClient({
  ...
  userPosts,
  registeredEvents,
  visibleBadges: initialVisibleBadges,
}: ProfileClientProps) {
  const router = useRouter()

  // State
  const [profile, setProfile] = useState<Profile | null>(initialProfile)
  const [visibleBadges, setVisibleBadges] = useState<VisibleBadge[]>(initialVisibleBadges)
  ...
  const [editForm, setEditForm] = useState({
    ...
    company_2: initialProfile?.company_2 || "",
    position_2: initialProfile?.position_2 || "",
    tagline: (initialProfile as any)?.tagline || "",
    introduction: initialProfile?.introduction || "",
    member_type: Array.isArray((initialProfile as any)?.member_type)
      ? (initialProfile as any).member_type
      : (initialProfile as any)?.member_type
        ? [(initialProfile as any).member_type]
        : [] as ("사업가" | "투자자" | "크리에이터")[],
    is_profile_public: initialProfile?.is_profile_public || false,
  })
              ...
              <div className="min-h-[300px]">
                {activeTab === "posts" && (
                  <div className="flex flex-col">
                    {userPosts.length > 0 ? (
                      userPosts.map((post) => (
                        <PostListItemComponent post={post} key={post.id} />
                      ))
                    ) : (
                  ...
                  <div className="flex flex-col">
                    {createdEvents.length > 0 ? (
                      createdEvents.map((event) => (
                        <EventListItemComponent event={event} key={event.id} />
                      ))
                    ) : (
